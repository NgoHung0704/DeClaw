...
- Current ticket: DCL-013 (next — Tool output parser + retry/repair)
...
- DCL-010/011/012 done — the agentic loop runs on a real Ollama-backed Mistral via `build_ollama_model` (`declaw/brain/chat_model.py`), wired into `build_agent_graph` through the `ModelCallable` seam. Unit tests fake `ChatOllama` at its boundary (no daemon); one integration test hits the live model and self-skips if Ollama is down. Next: DCL-013 — Tool output parser + retry/repair: when Mistral emits malformed/invalid tool-call output, retry with a corrective prompt (max 2). This is where 7B unreliability gets handled. Run `uv run pytest` and `uv run mypy declaw` before commits.
