...
- Current ticket: DCL-015 (next — Context compaction strategy on overflow)
...
- DCL-010..014 done — the loop runs on real Ollama Mistral with composable `ModelCallable` wrappers: `with_tool_call_repair` (DCL-013) and `with_context_window` (DCL-014, `declaw/brain/context.py`), both stacking over `build_ollama_model`. Next: DCL-015 — Context compaction strategy on overflow: instead of just dropping old turns, summarise the oldest N into a compact note (preserve task plan + recent). Builds on DCL-014's `fit_to_window` and the AgentState `scratchpad`/`plan` slots. Run `uv run pytest` and `uv run mypy declaw` before commits.
