...
- Current ticket: DCL-014 (next — Context window management (Mistral 32k))
...
- DCL-010..013 done — the agentic loop runs on a real Ollama Mistral (`build_ollama_model`) and can be hardened against bad tool calls via `with_tool_call_repair` (`declaw/brain/repair.py`), both composable through the `ModelCallable` seam. Next: DCL-014 — Context window management (Mistral 32k): a token counter with soft/hard caps + an eviction policy so long conversations never exceed 32k tokens (depends on DCL-011 AgentState). Run `uv run pytest` and `uv run mypy declaw` before commits.
