...
The model is *injected* as an async callable, so the loop is testable without
a live Ollama daemon. Wiring the real Mistral model (via langchain-ollama
``bind_tools``) is DCL-012; the richer typed ``AgentState`` is DCL-011.
"""
...
def build_agent_graph(
    *,
...
) -> CompiledStateGraph[MessagesState, Any, Any, Any]:
    ...
    async def think(state: MessagesState) -> dict[str, list[BaseMessage]]:
